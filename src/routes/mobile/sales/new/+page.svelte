...
<div class="p-4 space-y-6">
  ...
  <div class="bg-white rounded-lg p-4 shadow-sm border space-y-4">
    <div class="flex items-center justify-between">
      ...
      >
        {invoice?.customerInfo?.name ? '更换客户' : '选择客户'}
      </button>
    </div>
    ...
    {:else}
      <button
        on:click={() => showCustomerPicker = true}
        class="w-full p-3 border-2 border-dashed border-gray-300 rounded-lg text-gray-500 hover:border-red-300 hover:text-red-500 transition-colors"
      >
        点击选择客户
      </button>
    {/if}
  ...
  <div class="bg-white rounded-lg p-4 shadow-sm border space-y-4">
    ...
    <div>
      <label class="block text-sm font-medium text-gray-700 mb-1">
        制单人 <span class="text-red-500">*</span>
      </label>
      <input
        type="text"
      ...
      />
      {#if errors.createdBy}
        <p class="text-red-500 text-sm mt-1">{errors.createdBy}</p>
      {/if}
    </div>
